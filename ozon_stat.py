...
def visualize(day_ordered, day_delivered, day_ordered_pcs, day_delivered_pcs, filename, save=True):
    ...
    if save:
        fig.write_html(f"{filename.replace('.csv','.html')}", auto_open=True)

def __main__():
    filename = input()
    df = load_preprocess(filename)
    day_ordered, day_delivered, day_ordered_pcs, day_delivered_pcs = get_stat(df)
    visualize(day_ordered, day_delivered, day_ordered_pcs, day_delivered_pcs,filename,save=True)
