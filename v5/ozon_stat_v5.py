...
def __main__():
    ...
    stat_period = input('Enter group by type: {day, 7 days, 14 days, 28 days, week or month}')
    data = get_response(start_date,end_date)
    df, products_id = load_preprocess(data)
    print(f'Available products: {products_id}')
    product = input('Enter product to process')
    period, ordered, delivered, ordered_pcs, delivered_pcs = get_stat(df, start_date, end_date, stat_period=stat_period, product=product)
    visualize(period, ordered, delivered, ordered_pcs, delivered_pcs, save_path=f'{save_path}.html', product=product, stat_period=stat_period)
